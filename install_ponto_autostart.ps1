# Registra inicializacao automatica do RelogioPonto no boot/logon.
$taskName = "RelogioPontoBackground"
$appPath = $PSScriptRoot
if (-not $appPath) {
    $appPath = Split-Path -Parent $MyInvocation.MyCommand.Path
}
$scriptPath = Join-Path $appPath "auto_start_flask_v2.ps1"

if (-not (Test-Path $scriptPath)) {
    Write-Host "Script de auto-start nao encontrado: $scriptPath" -ForegroundColor Red
    exit 1
}
...
if (-not $registeredMode) {
    Write-Host "Nao foi possivel registrar a inicializacao automatica do RelogioPonto." -ForegroundColor Red
    exit 1
}
...
if ($registeredMode -eq "system") {
    ...
    Write-Host "O RelogioPonto sera iniciado automaticamente no boot e no logon." -ForegroundColor Green
    ...
    Write-Host "O RelogioPonto sera iniciado automaticamente quando este usuario fizer logon." -ForegroundColor Yellow
